...
<div class="min-h-screen bg-slate-100 py-6">
    <div class="mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-8">
        ...
        <nav class="mb-3 text-xs font-semibold text-slate-500">
            ...
            <span class="text-slate-900">Workspace #{{ $selectedProject->id }}</span>
        </nav>
                ...
                @else
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        ...
                        <div class="flex flex-wrap gap-2">
                            <button type="button" id="analyzeProjectBtn" class="rounded-xl bg-cyan-600 px-3 py-2 text-xs font-bold text-white hover:bg-cyan-700">Tao Prompt Anh (OpenAI) - Tat Ca Cau</button>
                    ...
                    <div class="mt-3 rounded-xl border border-cyan-200 bg-cyan-50 p-3">
                        <label for="analyzeCommonInstructionInput" class="mb-1 block text-[11px] font-bold uppercase tracking-wide text-cyan-800">Yeu cau chung khi tao prompt anh (ap dung cho tat ca cau)</label>
                        <textarea id="analyzeCommonInstructionInput" rows="2" class="w-full rounded-lg border-cyan-300 text-xs" placeholder="VD: uu tien tong mau am, camera close-up nhan vat chinh, tao cam giac co trang trang nghiem, tranh bo cuc qua nhieu vat the..."></textarea>
                        <p class="mt-1 text-[11px] text-cyan-700">He thong se goi OpenAI voi OPENAI_API_KEY trong env de tao image prompt cho toan bo cau.</p>
                    </div>
                    ...
                    @php
                        ...
                        $wizardSteps = [
                            [
                                ...
                                'action_js'    => $sentenceCount > 0
                                    ? ('window.location.href=\'' . route('media-center.edit', $selectedProject) . '\'')
                                    : ('window.location.href=\'' . route('media-center.create') . '\''),
                                'tip'   => 'Điền đầy đủ Thời đại, Thể loại và Bối cảnh thế giới để AI tạo prompt chính xác hơn.',
                            ],
                            [
                                'n' => 2, 'id' => 'analyze',
                                'label' => 'Tạo prompt ảnh AI',
                                'desc'  => 'Dùng OpenAI để tạo image prompt cho tất cả câu theo yêu cầu chung bạn nhập.',
                                'done'  => $hasAnalyzed,
                                'running' => $isAnalyzing,
                                'optional' => false,
                                'action_label' => $isAnalyzing ? 'Đang tạo prompt...' : ($hasAnalyzed ? 'Tạo lại prompt ảnh' : 'Bắt đầu tạo prompt ảnh'),
                                'action_js'    => 'document.getElementById(\'analyzeProjectBtn\').click()',
                                'tip'   => 'Bạn có thể nhập yêu cầu chung trước khi bấm nút để OpenAI đồng bộ phong cách prompt giữa các câu.',
                            ],
                            [
                                'n' => 3, 'id' => 'char_refs',
                                'label' => 'Ảnh nhân vật',
...
(function() {
    ...
    const analyzeCommonInstructionInput = document.getElementById('analyzeCommonInstructionInput');
    ...
    document.getElementById('analyzeProjectBtn')?.addEventListener('click', async () => {
        const btn = document.getElementById('analyzeProjectBtn');
        const commonInstruction = (analyzeCommonInstructionInput?.value || '').trim();
        ...
        const pollAnalyzeProgress = async () => {
            try {
                const data = await jsonFetch(`/media-center/projects/${projectId}/analyze/progress`, 'GET');
                const analyze = data?.analyze || {};
                const status = String(analyze?.status || '').trim().toLowerCase();
                const progress = Number(analyze?.progress || 0);
                const message = String(analyze?.message || '');
                const processed = Number(analyze?.processed_sentences || 0);
                const total = Number(analyze?.total_sentences || 0);

                if (status === 'queued') {
                    setStatus(`Đang chờ hàng đợi xử lý... (0/${total})`, 'normal');
                    return;
                }

                if (status === 'running') {
                    setStatus(`${message} — ${progress}% (${processed}/${total} câu)`, 'normal');
                    return;
                }

                if (status === 'completed') {
                    stopAnalyzePolling();
                    setStatus(`Tạo prompt ảnh xong (${processed}/${total} câu). Đang reload...`, 'success');
                ...
                if (status === 'failed') {
                    ...
                    setStatus('Lỗi tạo prompt ảnh: ' + (analyze?.error || message || 'Lỗi không xác định'), 'error');
                    return;
                }
            } catch (e) {
                stopAnalyzePolling();
                ...
                setStatus('Không poll được tiến trình: ' + e.message, 'error');
            }
        };

        setStatus('Đang đưa vào hàng đợi...');
        try {
            await jsonFetch(`/media-center/projects/${projectId}/analyze`, 'POST', {
                common_instruction: commonInstruction,
            });

            setStatus('Đã vào hàng đợi. Đang theo dõi tiến trình...');
            pollAnalyzeProgress();
            analyzePollTimer = setInterval(pollAnalyzeProgress, 2500);
        } catch (e) {
            if (btn) {
                btn.disabled = false;
                btn.classList.remove('opacity-70', 'cursor-not-allowed');
            }
            setStatus('Lỗi đưa vào hàng đợi: ' + e.message, 'error');
        }
    });
