...
class AudioBookSummaryController extends Controller
{
    ...
    /**
     * Pick which compression level (Bước 3) to generate with going forward. Existing
     * retells/outro already generated under the previous level are kept as-is — only
     * clusters not yet retold (or explicitly regenerated by the user afterward) pick up the
     * new target_level, since generateRetell() reads it fresh at call time.
     */
    ...
    public function saveVersion(Request $request, AudioBook $audioBook)
    {
        ...
        // Backstop for callers that don't send a label (or send an empty one) — the normal
        // path is the frontend prompt, already pre-filled with "[title] - Phiên bản N".
        $label = trim((string) $request->input('label')) ?: ($audioBook->title . ' - Phiên bản ' . (count($versions) + 1));
